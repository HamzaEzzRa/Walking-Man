...
{
	const uint32_t customAreaMusicOriginalMediaId = 14330364;
	...
	bool LoadCustomSongsFromFolder()
	{
		...
		for (const fs::directory_entry& audioFile : audioFiles)
		{
			try
			{
				...
				{
					Logging::Write(logPrefix, "Skipping custom audio with unprintable path: %s", Utils::PathToLogString(audioPath).c_str());
					continue;
				}
