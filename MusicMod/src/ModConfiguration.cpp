	...
	namespace Databases
	{
		std::unordered_map<std::string, FunctionData> functionDatabase =
		{
			...
			},
			{
				...
				"48 8D 05 ?? ?? ?? ?? C4 C1 78 29 7B B8 48 8B F9",
		...
		std::unordered_map<std::string, MusicData> sfxDatabase = {
			{
				"Silence",
				{100, MusicType::SFX, 0, "Silence", "",
				"?? ?? ?? ?? ?? 7F 00 00 E4 77 3B F0 67 6B 41 DB BB CC E6 75 2F 25 5B F2"}
			}
		};

		std::unordered_map<std::string, MusicData> unknownDatabase = {};
		std::unordered_map<std::string, MusicData> ambientDatabase = {};
		...
		// Music interruptors are UI sounds played through playUISoundFunction, stopping music in-game
		// Tracking them helps detect when autoplay should stop
		// Signature of each entry represents the values of the second arg passed to the play UI sound function
		std::unordered_map<std::string, MusicData> musicInterruptors =
		{
			{
				"Entering facility",
